...
public class SoundsAdapter extends RecyclerView.Adapter<SoundsAdapter.ViewHolder> {
    ...
    public SoundsAdapter(palarm palarm, List<SoundData> sounds) {
        this.palarm = palarm;
        this.sounds = sounds;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if (position == 0) {
            ...
            holder.icon.setOnClickListener(v -> {
                ...
                if (sound1.isPlaying(palarm) || currentlyPlaying == position1) {
                    sound1.stop(palarm);
                    currentlyPlaying = -1;
                } else {
                    sound1.preview(palarm);

                    if (currentlyPlaying >= 0) {
                        sounds.get(currentlyPlaying - 1).stop(palarm);
            ...
            setPlaying(holder, sound.isPlaying(palarm), false);
        }
    }
    ...
    private void setPlaying(final ViewHolder holder, final boolean isPlaying, final boolean isAnimated) {
        ...
        if (isAnimated) {
            AnimatedVectorDrawableCompat drawable = AnimatedVectorDrawableCompat.create(palarm, isPlaying ? R.drawable.ic_play_to_pause : R.drawable.ic_pause_to_play);
            if (drawable != null) {
