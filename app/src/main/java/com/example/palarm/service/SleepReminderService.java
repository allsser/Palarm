package com.example.palarm.service;
...
import com.example.palarm.data.PreferenceData;
...
public class SleepReminderService extends Service {
    ...
    public void refreshState() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? powerManager.isInteractive() : powerManager.isScreenOn()) {
            AlarmData nextAlarm = getSleepyAlarm(palarm);
            if (nextAlarm != null) {
     ...
     */
    @Nullable
    public static AlarmData getSleepyAlarm(palarm palarm) {
        if (PreferenceData.SLEEP_REMINDER.getValue(palarm)) {
            AlarmData nextAlarm = getNextWakeAlarm(palarm);
            if (nextAlarm != null) {
                Calendar nextTrigger = nextAlarm.getNext();
                nextTrigger.set(Calendar.MINUTE, nextTrigger.get(Calendar.MINUTE) - (int) TimeUnit.MILLISECONDS.toMinutes((long) PreferenceData.SLEEP_REMINDER_TIME.getValue(palarm)));
     ...
     */
    @Nullable
    public static AlarmData getNextWakeAlarm(palarm palarm) {
        ...
        List<AlarmData> alarms = palarm.getAlarms();
    ...
    public static void refreshSleepTime(Context context) {
        ...
        palarm palarm;
        if (context instanceof palarm)
            palarm = (palarm) context;
        else palarm = (palarm) context.getApplicationContext();

        if (getSleepyAlarm(palarm) != null)
            ContextCompat.startForegroundService(context, new Intent(palarm, SleepReminderService.class));
    }
