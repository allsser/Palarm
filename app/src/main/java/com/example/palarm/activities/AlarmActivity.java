...
import com.example.palarm.dialogs.TimeChooserDialog;
import com.example.palarm.palarm;
import com.example.palarm.service.SleepReminderService;
import com.example.palarm.utils.FormatUtils;
import com.example.palarm.utils.ImageUtils;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (sound != null && !sound.isPlaying(palarm))
                    sound.play(palarm);
        ...
        SleepReminderService.refreshSleepTime(palarm);
        ...
        new AlertDialog.Builder(AlarmActivity.this, isDark ? R.style.Theme_AppCompat_Dialog_Alert : R.style.Theme_AppCompat_Light_Dialog_Alert)
                .setItems(names, (dialog, which) -> {
                    if (which < minutes.length) {
                        TimerData timer = palarm.newTimer();
                        timer.setDuration(TimeUnit.MINUTES.toMillis(minutes[which]), palarm);
                        timer.setVibrate(AlarmActivity.this, isVibrate);
                        timer.setSound(AlarmActivity.this, sound);
                        timer.set(palarm, ((AlarmManager) AlarmActivity.this.getSystemService(Context.ALARM_SERVICE)));
                        palarm.onTimerStarted();
                        ...
                        timerDialog.setListener((hours, minutes1, seconds) -> {
                            ...
                            timer.set(palarm, ((AlarmManager) getSystemService(Context.ALARM_SERVICE)));
                            palarm.onTimerStarted();
                            finish();
                        });
                        timerDialog.show();
